...
def render_grid_html(grid, drones=None):
    if drones is None: drones = []
    drone_map = {}
    for d in drones:
        if d.position not in drone_map: drone_map[d.position] = []
        drone_map[d.position].append(d)
    ...
    for r in range(GRID_SIZE):
        ...
        for c in range(GRID_SIZE):
            ...
            if (r, c) in drone_map:
                content += f"<br>🚁x{len(drone_map[(r,c)])}"
...
with tab1:
    ...
    with col1:
        drones_to_show = st.session_state.sim_result.drone_states if st.session_state.sim_result else []
        grid_to_show = st.session_state.sim_result.sim_grid if (st.session_state.sim_result and st.session_state.sim_result.sim_grid) else st.session_state.grid
        st.markdown(render_grid_html(grid_to_show, drones_to_show), unsafe_allow_html=True)
...
with tab2:
    ...
    st.write(f"**Score: {score}/3 Rules Passed**")
        ...
        if "FAIL" in status:
            with st.expander("View Violations"):
                for v in val_res['violations']:
                    if r_id in v or (r_id == "R1" and "Industrial" in v) or (r_id == "R2" and "Residential" in v) or (r_id == "R3" and "Hub" in v):
...
with tab4:
    ...
    if st.button("🚀 LAUNCH SIMULATION", type="primary"):
        fleet_res = run_ga_fleet_selection(st.session_state.grid, st.session_state.budget)
        # Force exactly 2 drones to highlight anomaly and baseline
        fleet_res['light'] = 1
        fleet_res['heavy'] = 1
        ...
        from src.visualization import plot_route_map
        sim_res = None
        for step_res in run_simulation(st.session_state.grid, fleet_res, deliveries):
            sim_res = step_res
            ...
            if len(sim_res.event_log) > 0:
                progress_bar.progress(min(len(sim_res.event_log) / 20.0, 1.0))
                
            fig = plot_route_map(sim_res.sim_grid, sim_res.drone_states, sim_res.delivery_states)
            map_container.pyplot(fig)
            
            time.sleep(0.3) # Simulate real-time dynamically
            
        st_success.success("Simulation Complete!")

    if st.session_state.sim_result:
        st.markdown("#### Live Drone Routes Map")
        from src.visualization import plot_route_map
        res = st.session_state.sim_result
        fig = plot_route_map(res.sim_grid if res.sim_grid else st.session_state.grid, res.drone_states, res.delivery_states)
        st.pyplot(fig)
