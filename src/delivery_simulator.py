...
class SimulationResult:
    ...
    sim_grid: Optional[List[List[GridCell]]] = None

def run_simulation(grid: List[List[GridCell]], fleet_info: Dict[str, Any], initial_deliveries: List[Delivery]):
    event_log = []
    completed = 0
    delayed = 0
    failed = 0
    ...
    for step in range(1, 4):
        ...
        yield SimulationResult(copy.deepcopy(event_log), completed, delayed, failed, copy.deepcopy(drones), copy.deepcopy(deliveries), list(no_fly_activations), copy.deepcopy(sim_grid))
    ...
    for step in range(4, 7):
        ...
        if step == 6:
            ...
            log_event(step, "Applying A* Algorithm: Computed initial paths for assigned drones.")
        yield SimulationResult(copy.deepcopy(event_log), completed, delayed, failed, copy.deepcopy(drones), copy.deepcopy(deliveries), list(no_fly_activations), copy.deepcopy(sim_grid))
    ...
    for step in range(7, 11):
        ...
        log_event(step, f"Drones moving along routes (Step {step-6}/4).")
        yield SimulationResult(copy.deepcopy(event_log), completed, delayed, failed, copy.deepcopy(drones), copy.deepcopy(deliveries), list(no_fly_activations), copy.deepcopy(sim_grid))
            ...
            for drone in drones:
                if drone.status == "rerouting":
                    ...
                    if new_route_res["success"]:
                        ...
                        drone.status = "failed"
                        failed += 1
            log_event(step, "Applying A* Algorithm: Computed new paths bypassing (4,7).", level="success")
        if step == 14:
            for drone in drones:
                if drone.status == "delivering" and drone.route_index < len(drone.current_route) - 1:
                    drone.route_index += 1
    ...
    step = 19
    if victim_drone:
        log_event(step, f"Anomaly Detector classified BatteryAnomaly for {victim_drone.id}. Applying A* Algorithm for immediate Return to Base.", level="warning")
